...
void worker(int rank, char* name) {
	...
	while (true) {
		...
		if (state == Elevator) {
            dumpLog(1, rank, name, " is riding the elevator to floor ");
			...
			if (floorOfElevator == desiredFloor) {
                dumpLog(1, rank, name, " got off elevator and is now working on floor ");
			...
			//do work
			currWorkTick++;
            dumpLog(1, rank, name, " did some work on floor ");
			...
			if (currWorkTick >= workTicks) {
                dumpLog(1, rank, name, " is done with work on floor ");
